...
{	
	...
	//public float destroyDelayPrefab = 5f;
	...
	public Player playerScript; // its a script
	...
	//public AK.Wwise.Event stopAsteroidCollision;
	...
	
	{
		...
		if (compileSettings == true) 
		{
			...
			//tutorialActionModeText.SetActive(false);
		}

		Debug.Log("consentScreen.activeSelf" + consentScreen.activeSelf);

		if(consentScreen.activeSelf == false) // && infoScreenV1.activeSelf == false && infoScreenV2.activeSelf == false)
		{
		enableTutorial = true;
		}
		//levelCount = 0;
		...
		//string randomID = Random.Range(0,100000).ToString();
		...
		plane.transform.position = new Vector3(player.transform.position.x, player.transform.position.y, player.transform.position.z - 10);
		...
		//lastAngle = Mathf.Abs(UnityEditor.TransformUtils.GetInspectorRotation(listener.transform).z);
	...
	{	
		...
		// Debug.Log("buttopns pressed = "+ buttonsPressedThisStage );
		// Debug.Log("actions done " + actionsExecutedThisStage);
		// Debug.Log("spamIndexThisStage is " + spamIndexThisStage);

		// if (buttonsPressedThisStage != 0)
		// {

		// spamIndexThisStage = (float)actionsExecutedThisStage  / (float)buttonsPressedThisStage;

		// Debug.Log("calcualting...");
		// }


		// Debug.Log("objectCount = "+ objectCount);
		// Debug.Log("laserCount = "+ laserCount);
		// Debug.Log("alienCount = "+ alienCount);
		// Debug.Log("astronautCount = "+ astronautCount);
		// Debug.Log("asteroidCount = "+ asteroidCount);

		//Debug.Log("spawnCoolDown"+ spawnCoolDown);
		//Debug.Log("Stage"+ levelCount);
		// UI ELements Update
		...
		//Debug.Log("blackCurtain is" + blackCurtainActive);

		// CalculateSucessRatioThisStageLaser(lasersSpawnedThisStage, lasersDestroyedThisStage, lasersStruckThisStage);



		// Debug.Log("lasersSpawnedThisStage"+lasersSpawnedThisStage);
		// Debug.Log("lasersDestroyedThisStage"+lasersDestroyedThisStage);
		// Debug.Log("lasersStruckThisStage"+lasersStruckThisStage);
		// Debug.Log("SuccesRatelaser" + laserSucessRateThisStage );
		...
		{		
			...
			//killSoundLevelEnd.Post(gameObject);
		...
		// if (objectCount < spawnCap  && enableTutorial == false && timeLeft > 5 && inTransition == false)	
	...
	public void Reconfiguration() // gets called at awake and after "start game" // Sets gamemodespecific settings
	{
		...
		if (enableTutorial == true )  // sets up the tutorial WE MIGHT NOT NEED THAT HERE ( depends on where reconfiguation is called from) AS IT IS TRUE FROM THE BEGGINING AND WE ALWAYS START WITH TUTORIAL // ITS ONLY MENT AS CHECK TO SWITCH THE SPAWNER
		{
	...
	public void SkipStage() // never goes into Startgame to skip transition
	{
				...
				if (levelCount == 0) // always press the button please
				{
					...
					reportWriterScript.AppendToReportEmtpy();
					ResetStats();
					//levelCount++; // so report is consistent when skipping stages
					LevelTransition_1();
	...
	private void UpdateStats() // call this 
	{
		...
		// if ( levelCount == 0) // This catches stats from the Tutorial
		// {
		// 	buttonsPressedThisStage = buttonsPressed;
		// 	actionsExecutedThisStage = actionsExecuted;
		// 	hitsTakenThisStage = hitsTaken;
		// 	sucessfullHitsThisStage = sucessfullHits;
		// 	deltaRotationAngleThisStage = deltaRotationAngle;	
		// 	scoreLastStage = score;

		// 	asteroidsSpawnedThisStage = asteroidsSpawned;
		// 	lasersSpawnedThisStage = lasersSpawned;
		// 	astronautsSpawnedThisStage = astronautsSpawned;
		// 	aliensSpawnedThisStage = aliensSpawned;

		// 	asteroidsDestroyedThisStage = asteroidsDestroyed;
		// 	lasersDestroyedThisStage = lasersDestroyed;
		// 	astronautsDestroyedThisStage = astronautsDestroyed;
		// 	aliensDestroyedThisStage = aliensDestroyed;

		// 	asteroidsStruckThisStage = asteroidsStruck;
		// 	lasersStruckThisStage = lasersStruck;
		// 	astronautsStruckThisStage = astronautsStruck;
		// 	aliensStruckThisStage = aliensStruck;

		// 	asteroidSucessRateThisStage = asteroidSucessRate;
		// 	laserSucessRateThisStage = laserSucessRate;
		// 	astronautSucessRateThisStage = astronautSucessRate;
		// 	alienSucessRateThisStage = alienSucessRate;
	...
	{		
		...
		{	
			...
			//randomID.Substring(0,1)

			//Debug.Log("first char is " + randomID[0]);	
			...
			if (!string.IsNullOrEmpty(randomID) && randomID[0] == checkOne) // check what or whether something has been played before and then go straight to tutorial
			{
				enableV2 = true;
				enableTutorial = true;
		...
		public void GenerateRandomID()
	{
		...
		//idNumber = float.Parse(randomID);
		//Debug.Log("idNumber is " +idNumber);
	...
	static float UpdateObjectCount()
{
	...
	private void CalculateHeadRotation()
	{
		...
		//float rotationAngle = Mathf.Abs(UnityEditor.TransformUtils.GetInspectorRotation(listener.transform).z); // works only in the editor and not in game

		

		//if (IsInRange(angleZ, rotationAngle - 5, rotationAngle + 5) )

		if (rotationAngle >= lastAngle - 5 && rotationAngle <= lastAngle + 5)
		{	
			float angleZ = rotationAngle - Mathf.Abs(lastAngle);
			deltaRotationAngleThisStage = Mathf.Abs(deltaRotationAngleThisStage) + Mathf.Abs(angleZ);
		}
	...
	public void CalculateAllSuccessRatiosThisStage()
	{
		...
		CalculateSucessRatioThisStageAlien(aliensSpawnedThisStage, aliensDestroyedThisStage, aliensStruckThisStage);
		CalculateSucessRatioThisStageAstronaut(astronautsSpawnedThisStage, astronautsDestroyedThisStage, astronautsStruckThisStage);
		CalculateSucessRatioThisStageAsteroid(asteroidsSpawnedThisStage, asteroidsDestroyedThisStage, asteroidsStruckThisStage);
		CalculateSucessRatioThisStageLaser(lasersSpawnedThisStage, lasersDestroyedThisStage, lasersStruckThisStage);
	}
	...
	// public void Calc()
	// {	
	// 	float xdd = lasersSpawnedThisStage - ( lasersSpawnedThisStage - ( lasersDestroyedThisStage + lasersStruckThisStage));

	// 	if ( xdd != 0 )
	// 	{
	// 		laserSucessRateThisStage = lasersDestroyedThisStage / xdd;
	...
	private void StatsCalculator()
	{
			...
			{
			precisionScoreThisStage = (float)sucessfullHitsThisStage / (float)actionsExecutedThisStage;
			}
