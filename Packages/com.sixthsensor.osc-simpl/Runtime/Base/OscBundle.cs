...
namespace OscSimpl
{
	...
	public sealed class OscBundle : OscPacket
	{
		...
		public static bool TryReadFrom( byte[] data, int dataLength, ref int index, out OscBundle bundle )
		{
			...
			while( index < dataLength )
			{
				...
				if( TryReadFrom( data, endDataIndex, ref index, out subPacket ) ){
					//Debug.Log( "Sub packet read: " + index + " == " + endDataIndex );
					bundle.Add( subPacket );
				} else {
					if( OscGlobals.logWarnings ) Debug.LogWarning( logPrepend + "Failed to read packet.\nIndex: " + index + ", Packet size: " + packetSizeDataValue.intValue + ", Byte count: " + dataLength + ", Buffer size: " + data.Length );
					return false;
				}
			}
