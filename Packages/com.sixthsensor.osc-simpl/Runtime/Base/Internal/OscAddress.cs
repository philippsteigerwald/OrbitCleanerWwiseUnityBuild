...
namespace OscSimpl.Internal
{
	...
	public static class OscAddress
	{
		...
		static readonly char[] specialPatternCharacters = {
			...
			//charGroupRangeWildcard, // Minus character is not special on it's own.
		...
		/// <summary>
		/// Evaluates whether addressA matches addressB following OSC 1.0 Address Pattern Matching.
		...
		/// </summary>
		public static bool IsMatching( string addressA, string addressB )
		{
			...
			if( addressA[c1++] != addressPrefix ) return false;
			if( addressB[c2++] != addressPrefix ) return false;

			// Loop through all characters.
			char ch1;
			char ch2;
			while( c1 < addressA.Length && c2 < addressB.Length )
			{
				ch1 = addressA[c1++];
				ch2 = addressB[c2++];
				...
				if( ch1 == anyMultiWildcard || ch2 == anyMultiWildcard ) {
					while( c1 < addressA.Length ) if( addressA[c1++] == addressPrefix ) break;
					while( c2 < addressB.Length ) if( addressB[c2++] == addressPrefix ) break;
				...
				if( ch1IsBegin || ch2IsBegin ) {
					...
					if( ch1IsBegin ) {
						if( IsMatchingCharGroup( ch2, addressA, ref c1 ) ) continue; // Next two chars.
						return false; // No match
					}

					// maHasGroup
					if( IsMatchingCharGroup( ch1, addressB, ref c2 ) ) continue; // Next two chars.
					return false; // No match
				}
				...
				if( ch1IsBegin || ch2IsBegin ) {
					...
					if( ch1IsBegin ) {
						c2--;
						if( IsMatchingStringList( addressB, ref c2, addressA, ref c1 ) ) continue; // Next two chars.
						return false; // No match
					}

					// maHasGroup
					c1--;
					if( IsMatchingStringList( addressA, ref c1, addressB, ref c2 ) ) continue; // Next two chars.
					return false; // No match
				}
			...
			return c1 == addressA.Length && c2 == addressB.Length;
		}
